...
Tideform は、カードゲーム・ボードゲーム・アブストラクトゲーム・パズルなどのための、
...
```ts
import { createRenderer } from "@tideform/render";
...
const renderer = createRenderer<GameMeta>({
...
Tideform は「どう見えるか」と「どれを触ったか」を担当します。
...
ゲーム状態から `Scene` を生成する Presenter パターンを推奨しています。
...
function present(state: GameState): Scene<GameMeta> {
  return {
    items: [
      {
        type: "stack",
...
renderer.setScene(present(gameState));
```
